...
//            const std::vector<VertexID> &roots_master_local,
	...
	inline void one_to_all_raw_buffer(
			int root,
			void *raw_send,
			size_t ETypeSize,
			size_t size_buffer_send,
			void *&raw_recv,
			size_t &size_buffer_recv);
...
//            }
            ...
            // Send active masters to mirrors
            {
				...
				//MsgUnitBP *buffer_send = (MsgUnitBP *) malloc(end_que * sizeof(MsgUnitBP));
                for (VertexID que_i = 0; que_i < end_que; ++que_i) {
                    VertexID v_global = que[que_i];
                ...
                for (int root = 0; root < num_hosts; ++root) {
					...
					//MsgUnitBP *buffer_recv = nullptr;
					if (host_id == 3 && r_global == 2 && d == 7 && root == 3) {
						printf("host_id: %u buffer_send.size(): %lu buffer_send[1366].v_global: %u bytes: %lu\n", host_id, buffer_send.size(), buffer_send[1366].v_global, sizeof(MsgUnitBP) * buffer_send.size());
						//printf("host_id: %u s[1365]: %u s[1366]: %u s[1367]: %u\n", host_id, buffer_send[1365].v_global, buffer_send[1366].v_global, buffer_send[1367].v_global);
					}
//					{//test
//						size_t ETypeSize = sizeof(MsgUnitBP);
...
//							std::copy(buffer_send.begin(), buffer_send.end(), buffer_recv.begin());
//						}
...
//					void *raw_recv = nullptr;
//					size_t size_buffer_recv;
//					one_to_all_raw_buffer(
//							root,
//							buffer_send,
//							sizeof(MsgUnitBP),
//							end_que,
...
//						continue;
//					}
//					buffer_recv.resize(size_buffer_recv);
//					buffer_recv = (MsgUnitBP *) malloc(size_buffer_recv * sizeof(MsgUnitBP));
//					memcpy(buffer_recv, raw_recv, size_buffer_recv * sizeof(MsgUnitBP));
//					free(raw_recv);
                    ...
                    {// test
						if (r_global == 2 && d == 7 && root == 3) {
							printf("host_id: %u root: %u buffer_recv[1366].v_global: %u\n", host_id, root, buffer_recv[1366].v_global);
							//printf("host_id: %u root: %u buffer_recv.size(): %lu buffer_recv[1366].v_global: %u\n", host_id, root, buffer_recv.size(), buffer_recv[1366].v_global);
//							printf("host_id: %u r[1365]: %u r[1366]: %u r[1367]: %u\n", host_id, buffer_recv[1365].v_global, buffer_recv[1366].v_global, buffer_recv[1367].v_global);
							if (host_id == 3) {
								printf("host_id: %u buffer_send[1366].v_global: %u\n", host_id, buffer_send[1366].v_global);
							}
							exit(1);
						}
                    }
                    ...
                    {// test
						...
						if ((host_id == 1 || host_id == 2 || host_id == 0) && r_global == 2 && d == 7 && root == 3) {
							char hostname[1024];
...
//                    for (uint64_t b_i = 0; b_i < size_buffer_recv; ++b_i) {
//						const MsgUnitBP &m = buffer_recv[b_i];
                        VertexID v_global = m.v_global;
						if (r_global == 2 && d == 7 && root == 3) {
							printf("host_id: %u count: %lu v_global: %u\n", host_id, count, v_global);
						}
						count++;
                        if (!G.local_out_degrees[v_global]) {
                            continue;
...
//            /////////////////////////////////////////////////////////
//            //
//            // Host pushes labels locally
//            {
//                EdgeID start_index = 0;
//                for (const std::pair<VertexID, VertexID> e : buffer_send_indices) {
//                    VertexID v_head_global = e.first;
//                    EdgeID bound_index = start_index + e.second;
//                    if (G.local_out_degrees[v_head_global]) {
//                        local_push_labels(
		...
		size_t &size_buffer_recv)
{
    ...
    MPI_Bcast(&size_buffer_send,
              ...
              MPI_COMM_WORLD);
    message_time += WallTimer::get_time_mark();
	size_buffer_recv = size_buffer_send;
    if (!size_buffer_send) {
        return;
    }
	uint64_t bytes_buffer_send = size_buffer_send * ETypeSize;
	raw_recv = (char *) malloc(bytes_buffer_send);
	assert(raw_recv);
    // Broadcast the buffer_send
    message_time -= WallTimer::get_time_mark();
    if (host_id == root) {
		memcpy(raw_recv, raw_send, bytes_buffer_send);
    }
    if (bytes_buffer_send <= static_cast<size_t>(INT_MAX)) {
        // Only need 1 broadcast
		MPI_Bcast(raw_recv,
                  ...
                  root,
                  MPI_COMM_WORLD);
//		if (root == 3 && buffer_recv.size() == 1643) {
//			printf("host_id: %u after_bcast: buffer_recv.size(): %lu sizeof(E_T): %lu bytes_buffer_send: %lu\n", host_id, buffer_recv.size(), sizeof(E_T), bytes_buffer_send);
//		}
    } else {
        const uint32_t num_unit_buffers = ((bytes_buffer_send - 1) / static_cast<size_t>(INT_MAX)) + 1;
        const uint32_t unit_buffer_size = ((size_buffer_send - 1) / num_unit_buffers) + 1;
		const uint32_t unit_buffer_bytes = unit_buffer_size * ETypeSize;
        size_t offset = 0;
        for (uint32_t b_i = 0; b_i < num_unit_buffers; ++b_i) {
			uint32_t count = b_i == num_unit_buffers - 1
							? bytes_buffer_send - offset
							: unit_buffer_bytes;
			MPI_Bcast(reinterpret_cast<char *>(raw_recv) + offset,
					count,
					MPI_CHAR,
					root,
					MPI_COMM_WORLD);
			offset += unit_buffer_bytes;
