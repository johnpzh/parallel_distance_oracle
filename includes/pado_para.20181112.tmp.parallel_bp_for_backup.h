...
//					end_que);
		...
		// Process the queue
		{
			...
			vector< bitset<BITPARALLEL_SIZE> > tmp_active_v_roots(num_v, bitset<BITPARALLEL_SIZE>(0)); // correspond to active_v_roots
			weighti d = 0;
			while (0 != end_que) {
...
#pragma omp parallel for
				...
				// Sibling set and Child set prepare
...
#pragma omp parallel for
				...
				// Sibling
				vector<idi> tmp_siblings(num_r_id);
				vector<idi> sizes_tmp_siblings(end_que, 0);
				// Child
				vector<idi> tmp_children(num_r_id);
				vector<idi> sizes_tmp_children(end_que, 0);
				// Sibling root ids
				vector<inti> r_id_tmp_siblings(num_r_id);
//				vector<idi> sizes_r_id_tmp_siblings(end_que, 0);
				// Child root ids
				vector<inti> r_id_tmp_children(num_r_id);
//				vector<idi> sizes_r_id_tmp_children(end_que, 0);
					...
					for (idi i = i_start; i < i_bound; ++i) {
						...
						for (inti r_i = 0; r_i < BITPARALLEL_SIZE; ++r_i) {
							...
							if (d > tmp_d[tv][r_i]) {
								...
								// Siblings
								if (v < tv) { // ??? Why need v < tv !!! Because it's a undirected graph.
									idi &tmp_size = sizes_tmp_siblings[i_q];
									tmp_siblings[offsets_r_id[i_q] + tmp_size] = tv;
									r_id_tmp_siblings[offsets_r_id[i_q] + tmp_size] = r_i;
									++tmp_size;
...
//										que[que_h++] = tv; // Need parallel enqueue
		...
		//							tmp_d[tv][r_i] = d + 1;
		//						}
								idi &tmp_size = sizes_tmp_children[i_q];
								tmp_children[offsets_r_id[i_q] + tmp_size] = tv;
								r_id_tmp_children[offsets_r_id[i_q] + tmp_size] = r_i;
								++tmp_size;
...
#pragma omp parallel for schedule(dynamic)
				for (idi i_q = 0; i_q < end_que; ++i_q) {
					...
					idi i_s_bound = i_s_start + sizes_tmp_siblings[i_q];
					for (idi i_s = i_s_start; i_s < i_s_bound; ++i_s) {
						idi w = tmp_siblings[i_s];
						idi r_i = r_id_tmp_siblings[i_s];
...
//#pragma omp parallel for schedule(dynamic)
				for (idi i_q = 0; i_q < end_que; ++i_q) {
					...
					idi i_c_bound = i_c_start + sizes_tmp_children[i_q];
					for (idi i_c = i_c_start; i_c < i_c_bound; ++i_c) {
						idi c = tmp_children[i_c];
						idi r_i = r_id_tmp_children[i_c];
...
//						end_que
				...
				active_v_roots.swap(tmp_active_v_roots);
				++d;
			}
		}
