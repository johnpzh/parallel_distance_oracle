...
//		}
		...
		for (idi r_i = 0; r_i < bound_r_i; ++r_i) {
			...
			if (tmp_dist_r_t < dists_table[v_tail][r_root_id] && tmp_dist_r_t < SI_v_tail.candidates_dists[r_root_id]) {
				// Mark r_root_id as a candidate of v_tail
				if (CAS(&SI_v_tail.candidates_dists[r_root_id], (int16_t) WEIGHTILARGE_MAX, tmp_dist_r_t)) {
				...
				if (CAS(&dists_table[v_tail][r_root_id], (int16_t) WEIGHTILARGE_MAX, tmp_dist_r_t)) {
...
template <inti BATCH_SIZE>
inline weightiLarge WeightedParaVertexCentricPLL<BATCH_SIZE>::distance_query(
					idi v_id,
					idi cand_root_id,
...
template <inti BATCH_SIZE>
weightiLarge WeightedParaVertexCentricPLL<BATCH_SIZE>::query_distance(
								idi a,
								idi b)
{
	idi num_v = num_v_;
