...
private:
	...
	struct IndexType {
		...
		using dist_vid_pair = pair<weighti, idi>;
		priority_queue< dist_vid_pair,
			vector<dist_vid_pair>, 
			greater<dist_vid_pair> > labels; // A min heap
	...
	// Distance Tables
	{
		...
		for (idi r_root_id = 0; r_root_id < roots_size; ++r_root_id) {
			...
			for (idi l_i = 0; l_i < l_i_bound; ++l_i) {
				dists_table[r_root_id][Lr.labels[l_i].second] = Lr.labels[l_i].first;
			}
					...
					weighti tmp_dist_v_c)
{
	...
	for (idi i_l = 0; i_l < bound_i_l; ++i_l) {
		if (tmp_dist_v_c <= Lv.labels[i_l].first) {
			break;
		}
		idi r = Lv.labels[i_l].second;
		if (cand_real_id <= r || WEIGHTI_MAX == dists_table[cand_root_id][r]) {
			continue;
		}
		weighti label_dist_v_c = Lv.labels[i_l].first + dists_table[cand_root_id][r];
		...
		idi end_has_new_labels_queue)
{
	...
	for (idi r_roots_id = 0; r_roots_id < roots_size; ++r_roots_id) {
		...
		const IndexType &Lr = L[r_real_id];
		idi bound_i_l = Lr.labels.size();
		for (idi i_l = 0; i_l < bound_i_l; ++i_l) {
			dists_table[r_roots_id][Lr.labels[i_l].second] = WEIGHTI_MAX;
		}
		...
		vector<bool> &has_new_labels)
{
	for (idi i_q = 0; i_q < end_has_new_labels_queue; ++i_q) {
		...
		for (inti i_r = 0; i_r < bound_i_r; ++i_r) {
			...
			Lv.labels.push(make_pair(dist, r_root_id + roots_start));
			//test
