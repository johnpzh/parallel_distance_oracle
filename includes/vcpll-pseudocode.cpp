			...
			The current batch ID b_id)
{
	...
	if (got_labels[v_id]) {
		// If vertex v_id has already inserted labels in this batch
		// ++(L[v_id].batches.rbegin()->size);
		if (iter == L[v_id].distances.rbegin() -> dist) {
			// If the inserted label is in the same iteration, no need to add any new element in the distances array
			++(L[v_id].distances.rbegin() -> size);
		} else {
			...
			++(L[v_id].batches.rbegin()->size);
			L[v_id].distances.push_back(DistanceIndexType(L[v_id].vertices.size(), 1, iter));
		}
	} else {
				...
				The index L)
{
	...
	A index recording the head of active_queue is head_active_queue = 0;
	...
	A reference of active_queue is ref_active_queue = active_queue;
	A reference of candidate_queue is ref_candidate_queue = candidate_queue;
	...
	while (end_active_queue != 0) { // active_queue is not empty
		...
		end_candidate_queue = 0;
		A temporary reference tmp_ref = ref_active_queue;
		ref_active_queue = ref_candidate_queue;
		ref_candidate_queue = tmp_ref;
