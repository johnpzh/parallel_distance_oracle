...
// The function should be called at the end of every batch processing.
void filter_out_labels(
		A queue vector<idi> has_new_labels_queue, // A queue recording all vertices which have got new labels in this batches
		The candidate distances data structure vector<ShortIndex> short_index, // every vertex has a label queue and a distance array for new labels in this batch
		The ID of beginning vertex of this batch idi roots_start // Just for ID transfer
		)
{
	// Traverse has_new_labels_queue for every vertex who got new labels in this batch
	for (every vertex v_id in has_new_labels_queue) {
		// Traverse v_id's every new label
		for (every label c_root_id in short_index[v_id].vertices_que) {
			Distance of the label (from v to c) dist_v_c = short_index[v_id].vertices_dists[c_root_id];
			// Traverse v_id's other new label to see if has shorter distance
			for (every label r_root_id in short_index[v_id].vertices_que) {
				if (c_root_id <= r_root_id) {
					// r_root_id should be higher ranked to have label c_root_id
					continue;
				}
				Distance from v to r dist_v_r = short_index[v_id].vertices_dists[r_root_id];
				if (dist_v_r > dist_v_c) {
					// Distance from v to r should not larger than dist_v_c
					continue;
				}
				Distance from c to r dist_c_r = short_index[c_root_id + roots_start].vertices_dists[r_root_id];
				if (dist_c_r > dist_v_c) {
					// Distance from c to r should not larger than dist_v_c
					continue;
				}
				// Check the distance relation
				if (dist_v_r + dist_c_r <= dist_v_c) {
					// Shorter distance exits, then label c is not necessary
					short_index[v_id].vertices_dists[c_root_id] = INF; // Filter out c_root_id from v_id's labels
				}
			}
		}
	}
...
//	The label table is vector< vector<weighti> > labels_table(num_v, vector<weighti>(roots_size, INF));
	...
	static vector<bool> has_new_labels(num_v, false);
		// This queue is added at last; it is used in the implementation but not in this pseudocode so far. Needs update later.
